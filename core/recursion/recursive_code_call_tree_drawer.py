...
# The user can enter Python code with recursive functions and an initial call.
# The script will add a decorator to the functions and execute the initial call.
# A decorator is a function that takes another function as input and returns a modified version of it.
# The call tree is then drawn using the tkinter library.
# The user can then click on a node to see the code of the function.
...
class CallTreeNode:
    ...
    def __init__(
        self, func_name, args, kwargs, parent_id=None, call_id=None
    ):  # Added call_id
    ...
    def _create_display_str(self):
        try:
            ...
            if self.return_value is not None and not (
                isinstance(self.return_value, str)
                and self.return_value.startswith("Error:")
            ):
...
class RecursiveTracer:
    ...
    def trace_calls(self, func):
            ...
            try:
                ...
                if self.call_stack:
                    ...
                    if finished_id != node.id:
                        ...
                        print(
                            f"Warning: Call stack mismatch! Expected {node.id}, popped {finished_id}"
                        )
        ...
        try:
            ...
            pass  # Ignore if signature cannot be created
...
def layout_tree(node, current_depth, node_font):
    ...
    children_subtree_widths = [
        layout_tree(child, current_depth + 1, node_font) for child in node.children
    ]
    total_children_width = (
        sum(children_subtree_widths) + max(0, len(node.children) - 1) * NODE_H_SPACE
    )
...
def position_nodes(node, current_x, node_font):
    ...
    if node.children:
        children_total_width = (
            sum(child.subtree_width for child in node.children)
            + max(0, len(node.children) - 1) * NODE_H_SPACE
        )
...
def draw_tree(canvas, node, node_font):
    ...
    canvas.create_rectangle(
        x0, y0, x1, y1, fill="lightblue", outline="black", width=NODE_BORDER_WIDTH
    )
    canvas.create_text(
        node.x, node.y, text=node.display_str, font=node_font, anchor=tk.CENTER
    )
    for child in node.children:
        if child:
...
class CallTreeVisualizer(tk.Tk):
    def __init__(self):
        ...
        # --- Code Input Area ---
        tk.Label(self, text="Enter Python Code (including recursive functions):").grid(
            row=0, column=0, sticky="w", padx=5, pady=2
        )
        self.code_text = scrolledtext.ScrolledText(
            self, height=10, width=80, wrap=tk.WORD, font=("Consolas", 10)
        )
        self.code_text.grid(row=1, column=0, padx=5, pady=5, sticky="nsew")
        # Default text updated
        self.code_text.insert(
            tk.END,
            """# Example: Palindrome Check (with nested function)
...
""",
        ...
        tk.Label(
            control_frame, text="Initial Call (e.g., is_palindrome('racecar')):"
        ).pack(side=tk.LEFT, padx=5)
        self.call_entry = tk.Entry(
            control_frame, width=40, font=("Consolas", 10)
        )  # Wider entry
        ...
        self.run_button = tk.Button(
            control_frame, text="Visualize Call Tree", command=self.run_visualization
        )
    ...
    def run_visualization(self):
        ...
        if not user_code or not initial_call:
            messagebox.showerror(
                "Error", "Please provide both Python code and an initial call."
            )
            return
        ...
        exec_namespace = {decorator_internal_name: self.tracer.trace_calls}
        ...
        try:
            ...
            messagebox.showerror(
                "Syntax Error",
                f"Error parsing code (potentially after adding decorators):\n{e}",
            )
            ...
            messagebox.showerror(
                "Runtime Error", f"Error during execution:\n{type(e).__name__}: {e}"
            )
        ...
        if not call_tree_root:
            ...
            if not self.tracer.all_nodes:
                messagebox.showinfo(
                    "Info",
                    "No decorated function calls were traced. Was the initial call correct?",
                )
            else:
                # This might happen if exec(initial_call) raised an error *before* any decorated func was entered
                messagebox.showwarning(
                    "Warning",
                    "Execution finished, but no call tree root was established. Drawing may be empty or incomplete.",
                )
            return
        ...
        try:
            ...
            position_nodes(
                call_tree_root, canvas_width_offset, self.node_font
            )  # Pass offset only once

            # 3. Find overall bounds *after* positioning
            min_x, max_x = float("inf"), float("-inf")
            min_y, max_y = float("inf"), float("-inf")
            if (
                not self.tracer.all_nodes
            ):  # Handle case where root exists but no nodes (shouldn't happen?)
                messagebox.showerror(
                    "Error", "No nodes found for drawing despite having a root."
                )
                return
            ...
            self.canvas.configure(
                scrollregion=(scroll_x0, scroll_y0, scroll_x1, scroll_y1)
            )
            ...
            messagebox.showerror(
                "Drawing Error",
                f"Error laying out or drawing the tree:\n{type(e).__name__}: {e}",
            )
